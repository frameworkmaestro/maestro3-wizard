...
class MWizardScript {
        ...
        foreach ($this->ini as $className => $node) {
            ...
            if ($tableName) {
                ...
                foreach ($attributes as $attributeName => $attributeData) {
                    ...
                    if ($lowerAttrType == 'collection') {
                        $setterBody = "if (!(\$value instanceof \\MArray)) {\n            \$value = new \\MArray(\$value);\n        }\n        ";
                    } elseif ($lowerAttrType == 'currency') {
                    ...
                    } elseif ($lowerAttrType == 'blob') {
                        $setterBody = "if (!(\$value instanceof \\MFile)) {\n            \$value = \\MFile::path(\$value);\n        }\n        ";
            ...
            $template->applyClass();
            $template->saveResult("{$moduleName}/models/map/{$className}{$this->getMapSufix()}", $this->baseDir);
    ...
    public function generateEnumeration($className, $var) {
        ...
        $template->applyClass();
        $template->saveResult("{$var['moduleName']}/models/map/{$className}{$this->getMapSufix()}", $this->baseDir);
    ...
    private function getMapSufix() {
        $mapSufix = ($this->keepModelNames) ? "Map" : "map";
        return "{$mapSufix}.php";
    }
