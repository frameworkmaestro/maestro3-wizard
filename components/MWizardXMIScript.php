...
class MWizardXMIScript {
    ...
    public function setKeepModelNames($keepModelNames)
    {
        $this->keepModelNames = $keepModelNames;
    }
    ...
    private function handleAssociationClass() {
        foreach ($this->nodes['uml:AssociationClass'] as $idA => $assoc) {
            ...
            if ((count($association) == 0) ||
                ...
                $this->nodes['AssociativeClass'][$class0][$idA] = array(strtolower($assoc->getAttributeNode('name')->value), $idA, $associationName);
                $this->nodes['AssociativeClass'][$class1][$idA] = array(strtolower($assoc->getAttributeNode('name')->value), $idA, $associationName);
    ...
    function handleClass($elements) {
        ...
        $namePrefix = (!empty($this->schemaName)) ? "{$this->schemaName}." : "";
        ...
        foreach ($classNodes as $node) {
            ...
            if ($t = $this->getChild($node->firstChild->nextSibling, 'ormDetail')) {
                ...
                if ($table->nodeType == XML_ELEMENT_NODE) {
                    ...
                    $document[] = "table = \"{$namePrefix}{$tableName}\"";
                }
            }
                            ...
                            if ($c->getAttributeNode('xmi:value')->value == 'Attribute_PK_id') {
                                ...
                                if (!$noGenerator) {
                                    if ($idIdentity) {
                                        ...
                                        $attribute .= ",{$namePrefix}seq_{$tableName}";
                                    }
                                }
                            }

                            if ($c->getAttributeNode('xmi:value')->value == 'Attribute_UID_id') {
                                $attribute .= ",not null,uid";
                            }
                        }
            ...
            if (count($this->nodes['AssociativeClass'][$id])) {
                foreach ($this->nodes['AssociativeClass'][$id] as $idA => $association) {
                    ...
                    $docattr[] = "associations['{$associationName}'] = \"{$module}\models\\{$toClass},oneToMany,{$pk}:{$pk}\"";
                }
            }
    ...
    private function createAssociationNode($association, $myself, &$attribute, &$params) {
        ...
        if ($to->nodeType == XML_ELEMENT_NODE) {
            ...
            if ($from == '') {
                $from = $this->nodes['uml:AssociationClass'][$myself->getAttributeNode('type')->value];
            }
            $fromClass = $from->getAttributeNode('name')->value;
            if (!$this->keepModelNames) {
                $toClass = strtolower($toClass);
                $fromClass = strtolower($fromClass);
            }
            $autoAssociation = ($toClass == $fromClass);
        }
        if ($this->ddd) {
            $at .= "{$module}\\persistence\\maestro\\{$toClass}\\{$toClassName}Map";
        } else {
        ...
        if ($cardinality == 'manyToMany') {
            ...
            $tableId = $relNode->getAttributeNode('to')->value;
            $tableNode = $this->nodes['dbTable'][$tableId];
            $tableName = $params['namePrefix'] . $tableNode->getAttributeNode('name')->value;
            $at .= ",{$tableName}\"";
